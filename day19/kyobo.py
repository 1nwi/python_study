...
for book in best_list:
    ...
    # main_author, sub_author = author_name_list  이 코드를 이용하면 저자 더보기가 없는 목록들에 의해 오류가 일어나서 if문으로 조건을 걸어줌
    if len(author_name_list) == 1:
        main_author = author_name_list[0]
        sub_author = ''
    else :
        main_author, sub_author = author_name_list
    # main_author = author_name_list[0]   두개를 main_author, sub_author = author_name_list 처럼 합칠수 있음
    # sub_author = author_name_list[1]
